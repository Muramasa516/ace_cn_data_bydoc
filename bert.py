...
def ch_ACE_reader(path, set_type):
    tokens_set = []  # [['a','b'],[]..]
    labels_set = []  # [['O','I-'],[]..]
    label_set = []
    label_set.append('O')
    with open(path, 'r',encoding='utf-8') as fin:
        ...
        for data_raw in lines:
            ...
            if len(raw)<2 or raw=='\n':
                continue
            tokens = raw[0].split(' ')# [["我们",".."],[]]
            sentence = raw[0].replace(' ','')
            label = raw[1].split(' ')
            chars = [char for char in sentence]#['我','们']
            labels = ['O']*len(chars)  # [["...",".."],[]]
            j=0
            ...
            tokens_set.append(chars)
            labels_set.append(labels)

    return tokens_set, labels_set, list(OrderedDict.fromkeys(label_set))


class NER_Dataset(data.Dataset):
    def __init__(self, tag2idx, sentences, labels, tokenizer_path='bert_chinese/', do_lower_case=True):
    ...
    def __getitem__(self, idx):
        ...
        if len(token_ids) > 127:
            ...
            modified_labels = modified_labels[:128]
...
class Bert(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attn_masks, labels=None):  # dont confuse this with _forward_alg above.
        ...
        outputs=(emission,)+outputs[2:]
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            if attn_masks is not None:
                active_loss = attn_masks.view(-1) == 1
                active_logits = emission.view(-1, self.num_labels)
        ...
        #     loss = criterion(emission.view(-1, self.num_labels), labels.view(-1))
        #     loss = loss * attn_masks.view(-1)
        #     loss = torch.sum(loss)/(torch.sum(attn_masks)+1e-5)
        #     return loss
        # else:
        #     emission=emission.detach().cpu().numpy()
        #     prediction = np.argmax(emission, axis=2)
        #     return prediction


def generate_training_data(config, bert_tokenizer="bert-base-uncased", do_lower_case=True):
    # 读入文件
    training_data, validation_data = config.data_dir + config.training_data, config.data_dir + config.val_data
    train_sentences, train_labels, label_set = ch_ACE_reader(training_data, 'train')
    label_set.append('X')
    ...
    dev_sentences, dev_labels, _ = ch_ACE_reader(validation_data, 'dev')
...
def generate_test_data(config, tag2idx, bert_tokenizer="bert_chinese/", do_lower_case=True):
    test_data = config.data_dir + config.test_data
    test_sentences, test_labels, _ = ch_ACE_reader(test_data, 'test')
...
def train(train_iter, eval_iter, tag2idx, config, bert_model="bert_chinese/"):
    # print('#Tags: ', len(tag2idx))
    unique_labels = list(tag2idx.keys())
    model = Bert.from_pretrained(bert_model, num_labels=len(tag2idx))
    model.train()
    ...
    for epoch in (train_iterator):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if step == 0:
                ...
                str(datetime.datetime.now())[5:19], (step + 1), len(epoch_iterator), loss.item()))
            if (step + 1) % 100 == 0:
                print('%s Step: %d of %d Loss: %f' % (
                str(datetime.datetime.now())[5:19], (step + 1), len(epoch_iterator), tmp_loss / 1000))
                tmp_loss = 0.0
...
def usage(parameter):
    ...
    print("Example usage (training):\n", \
          "\t python bert_cbloss.py --mode train ")

    print("Example usage (testing):\n", \
          "\t python bert_cbloss.py --mode test ")
